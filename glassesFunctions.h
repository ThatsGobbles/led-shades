...
byte qsine(int angle) {
  ...
  if (cangle < 90) {
    return pgm_read_byte(&QsineLookupTable[cangle]);
  } else if (cangle < 180) {
    return 255-pgm_read_byte(&QsineLookupTable[179-cangle]);
  } else if (cangle < 270) {
    return 255-pgm_read_byte(&QsineLookupTable[cangle-180]);
  } else {
    return pgm_read_byte(&QsineLookupTable[359-cangle]);
  }
